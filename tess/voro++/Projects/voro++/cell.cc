...
void voronoicell_base::output_vertices(FILE *fp) {
	if(p>0) {
		fprintf(fp,"(%.12f,%.12f,%.12f)",*pts*0.5,pts[1]*0.5,pts[2]*0.5);
		for(double *ptsp=pts+3;ptsp<pts+3*p;ptsp+=3) fprintf(fp," (%.12f,%.12f,%.12f)",*ptsp*0.5,ptsp[1]*0.5,ptsp[2]*0.5);
	}
}
...
void voronoicell_base::output_vertices(double x,double y,double z,FILE *fp) {
	if(p>0) {
		fprintf(fp,"(%.12f,%.12f,%.12f)",x+*pts*0.5,y+pts[1]*0.5,z+pts[2]*0.5);
		for(double *ptsp=pts+3;ptsp<pts+3*p;ptsp+=3) fprintf(fp," (%.12f,%.12f,%.12f)",x+*ptsp*0.5,y+ptsp[1]*0.5,z+ptsp[2]*0.5);
	}
}
	...
	while(*fmp!=0) {
		if(*fmp=='%') {
			...
			switch(*fmp) {
				...
				case 'x': fprintf(fp,"%.12f",x);break;
				case 'y': fprintf(fp,"%.12f",y);break;
				case 'z': fprintf(fp,"%.12f",z);break;
				case 'q': fprintf(fp,"%.12f %.12f %.12f",x,y,z);break;
				case 'r': fprintf(fp,"%.12f",r);break;
				...
				case 'm': fprintf(fp,"%.12f",0.25*max_radius_squared());break;

				// Edge-related output
				case 'g': fprintf(fp,"%d",number_of_edges());break;
				case 'E': fprintf(fp,"%.12f",total_edge_distance());break;
				case 'e': face_perimeters(vd);voro_print_vector(vd,fp);break;

				// Face-related output
				case 's': fprintf(fp,"%d",number_of_faces());break;
				case 'F': fprintf(fp,"%.12f",surface_area());break;
				case 'A': {
						  face_freq_table(vi);
				...
				case 'v': fprintf(fp,"%.12f",volume());break;
				case 'c': {
						  double cx,cy,cz;
						  centroid(cx,cy,cz);
						  fprintf(fp,"%.12f %.12f %.12f",cx,cy,cz);
					  } break;
				case 'C': {
						  double cx,cy,cz;
						  centroid(cx,cy,cz);
						  fprintf(fp,"%.12f %.12f %.12f",x+cx,y+cy,z+cz);
					  } break;
