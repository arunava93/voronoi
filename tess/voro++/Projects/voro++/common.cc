...
void voro_print_vector(std::vector<double> &v,FILE *fp) {
	int k=0,s=v.size();
	while(k+4<s) {
		fprintf(fp,"%.12f %.12f %.12f %.12f ",v[k],v[k+1],v[k+2],v[k+3]);
		k+=4;
	}
	if(k+3<=s) {
		if(k+4==s) fprintf(fp,"%.12f %.12f %.12f %.12f",v[k],v[k+1],v[k+2],v[k+3]);
		else fprintf(fp,"%.12f %.12f %.12f",v[k],v[k+1],v[k+2]);
	} else {
		if(k+2==s) fprintf(fp,"%.12f %.12f",v[k],v[k+1]);
		else fprintf(fp,"%.12f",v[k]);
	}
}
